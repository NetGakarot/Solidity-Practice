...
revert(string memory reason) – This method aborts the execution and reverts any changes done to the state.
...
contract LearnErrorHandling {
    ...
    function internalTesting() public view {
        assert(finalCalc != 6);
    }
...
2. Add a function becomeSeller which sets the seller to the msg.sender.balance;
3. Create a function named sell which is payable and check to see
if an input of an amount is greater thatn msg.value
to revert that there is not enough ether provoided as an throw error
*/
